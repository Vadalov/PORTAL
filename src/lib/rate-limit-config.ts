...
export const RATE_LIMIT_CONFIGS: EndpointConfig[] = [
  ...
  {
    pattern: /^\/api\/(beneficiaries|users|tasks|meetings|messages|donations)\/[^\/]+\/(PUT|DELETE|PATCH)/,
    rateLimitFunction: dataModificationRateLimit,
    description: 'Data modification endpoints - 50 requests per 15 minutes'
  },
  {
    pattern: /^\/api\/(beneficiaries|users|tasks|meetings|messages|donations)\/[^/]*\/(POST|PUT|DELETE)/,
    rateLimitFunction: dataModificationRateLimit,
    description: 'Data modification endpoints - 50 requests per 15 minutes'
  },
...
export function applyGlobalRateLimit(
  ...
  const matchedConfig = RATE_LIMIT_CONFIGS.find(config => 
