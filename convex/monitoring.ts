...
export const getPerformanceMetrics = query({
  ...
  handler: async (ctx, args) => {
    ...
    let metrics = await ctx.db.query("performance_metrics").order("desc").take(limit * 2);
...
export const getErrorLogs = query({
  ...
  handler: async (ctx, args) => {
    ...
    let logs = await ctx.db.query("error_logs").order("desc").take(limit * 2);
...
export const getSystemStats = query({
  ...
  handler: async (ctx) => {
    const [
      ...
      ctx.db.query("beneficiaries").collect().then((r) => r.length),
      ctx.db.query("donations").collect().then((r) => r.length),
      ctx.db.query("tasks").collect().then((r) => r.length),
      ctx.db.query("meetings").collect().then((r) => r.length),
      ctx.db
        .query("error_logs")
