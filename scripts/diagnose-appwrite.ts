...
async function main() {
  ...
  if ((!section || section === 'connectivity') && provider === 'appwrite') {
    ...
    try {
      ...
      if (!jsonOutput) {
        ...
        console.log(
          `Tests: ${connectivityReport.summary.successfulTests}/${connectivityReport.summary.totalTests} passed`
        );
      }
    } catch (error: any) {
      overallScore -= 30;
  ...
  if ((!section || section === 'mock') && provider === 'mock') {
    ...
    if (!jsonOutput) {
      console.log(
        `Schema: ${schemaReport.summary.validCollections}/${schemaReport.summary.totalCollections} valid`
      );
  ...
  const report = {
    ...
    documentationLinks: ['docs/APPWRITE_SETUP.md', 'docs/CONFIGURATION-TROUBLESHOOTING.md'],
    results,
    environment: {
